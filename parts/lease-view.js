...
function LeaseView({ lease }) {
    return <div className='p-1 max-w-4xl md:mx-auto'>
        ...
        <ViewArticle caption='二. 租住及约定'>
            <p>
                <span>{lease.tenancy.share ? '本合同限定为合租，乙方租用部分房间，与他人共用设施' : '本合同限定为整租，乙方独享使用权'}, </span>
                <span>{lease.tenancy.restrict ? `${lease.tenancy.restrict}, ` : ''}</span>
                限住<span className='px-1'>{lease.tenancy.peopless}</span>人。
            </p>
            <p className='py-1'>
                租赁期内，乙方不得在厨房、卫生间等非居住空间安排住宿，否则视为乙方违约。
            </p>
        </ViewArticle>
        <ViewArticle caption='三. 租赁期限'>
            <p>
                ...
                甲方应于合同生效之日起将房屋交付给乙方使用，租赁期满后，甲方收回房屋。
            </p>
            <p>
        ...
        <ViewArticle caption='四. 租金、滞纳金与押金'>
            ...
            {lease.lateFee?.amount ? <p className='py-1'>
                如乙方无法按期及时支付租金，甲方有权收取滞纳金，金额为每日<span className='px-1'>{lease.lateFee.amount}</span>元
                (<span className='px-1'>{lease.lateFee?.sinote}</span>)。
                {lease.lateDays &&
                    <span className='px-1'>{`最多收取${lease.lateDays}天（滞纳期限）后，如乙方仍然无法支付租金，甲方有权收回房屋，不退押金。`}</span>
                }
            </p> : ''}
        ...
        <ViewArticle caption='六. 设施与维护'>
            ...
            <p className='py-1'>
                {lease.lessor?.provides
                    ? `甲方提供并负责维护的设备和家电包括：${lease.lessor.provides}，如果甲方不能对出了问题的设备和家电及时提供维修或维护，
                    乙方有权自行解决问题，费用由甲方支付。`
                    : '各种生活设施（包括电器）由乙方自行维护。'}
            </p>
        </ViewArticle>
        <ViewArticle caption='七. 杂费支付'>
            ...
            <p className='py-1'>乙方入住前，甲方应付清此前房屋所涉及的水电、物业管理等所有费用，租赁期间发生的其他未明确的费用由甲方支付。</p>
        </ViewArticle>
        <ViewArticle caption='八. 转让与转租'>
            <p>
                租赁期内，如房屋产权发生变更或转让，甲乙双方应会同新房主协商解决租约接续问题，如无法解决，甲方应无条件退还乙方剩余房租及押金，
                另外支付与押金等额的赔偿作为对乙方的补偿。
            </p>
            <p className='py-1'>租赁期间，甲方{lease.lessee?.sublet
                ? '允许乙方分租、转租所承租的房屋并告知甲方，房屋原租期不变，乙方自行处理与新租户的租约，且不得违反本合同所约定的条款。'
                : '不允许乙方分租、转租所承租的房屋。'}
            </p>
        </ViewArticle>
        <ViewArticle caption='九. 合同的变更与解除'>
            <p>
                如果甲方或乙方在租赁期间不能遵守前述各项条款，则对方有权解约并索赔。
            </p>
            <p className='py-1'>
        ...
        <ViewArticle caption='十. 违约与免责'>
            ...
            <p className='py-1'>
                租赁期内，乙方如果滋事扰民、在房屋内从事违法犯罪活动，责任将自行承担，甲方因此类活动有权解约、收回房屋并没收剩余租金与押金。
            </p>
        </ViewArticle>
